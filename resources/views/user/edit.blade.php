...
@section('content')
    <form method="post" action="{{url('admin/user/'.$type->id)}}">
        ...
        Brand:<input type="text" name=" brand" value="{{$type->getBrand($type->parent_id)->name}}"><br/>
        Series:<input type="text" name="series" value="{{$type->getSeries($type->parent_id)->name}}"><br/>
        Type:<input type="text" name="type" value="{{$type->name}}"><br/>
